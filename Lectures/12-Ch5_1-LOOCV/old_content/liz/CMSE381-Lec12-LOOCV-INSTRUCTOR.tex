...
\centering
\InstructorList{
  \item Previous versions of the course with more time I talked about the special case with linear regression. Skipping that this year (2024) because 10 minutes shorter is making everything tight. The slides are still commented out below 
	\item Outliers
...
\end{frame}
%--- Next Frame ---%
\begin{frame}[c]{Throw-back Monday}

	\centering
	\includegraphics[width = .8\textwidth]{Fig2-11}
...
\item Doesn't shake up the data enough: estimates from each fold are highly correlated so their averages have high variance.
}
\end{columns}
