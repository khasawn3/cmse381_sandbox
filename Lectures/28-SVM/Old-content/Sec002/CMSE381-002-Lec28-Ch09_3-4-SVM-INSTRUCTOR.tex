...
\centering
	...
	\textbf{This lecture:}
	\begin{itemize}
		\item 9.3 Support Vector Machine
\end{itemize}
...
\includegraphics[align = c, width = \textwidth]{Schedule-F25-3.png}
\end{columns}
...
\begin{frame}[t]{Hyperplane becomes a classifier }
...
\begin{frame}[t]{How do we pick? Old version}
...
Test your understanding in \href{https://PollEv.com/multiple_choice_polls/qqBBddUTxHVAKw5iIJqRc/respond}{PollEv}!
\InstructorList{
  \item Soft margin
...
\begin{itemize}
\item How is nonlinearity introduced in Support Vector Machine?
	\begin{itemize}
	\item You should be able to describe how kernels work conceptually and mathematically.
	\end{itemize}
\item What type of kernel makes Support Vector Machine equivalent to Support Vector Classifier?
\item What other kernels are there?
\item What type of kernel leads to more local behavior?
\item How to use Support Vector Machine in Python?
	\begin{itemize}
	\item You should be able to fit a Support Vector Machine using different kernels covered in class.
	\item You should be able select the appropriate hyperparameter that optimize bias-variance trade off.
	\end{itemize}
\end{itemize}
...
\begin{frame}[t]{How to bend the plane: polynomial features?}
...
\begin{frame}[t]{Kernels - "There is no plane" }
...
\column{.45\textwidth}
\centering
\includegraphics[width=0.7\textwidth]{No_spoon.jpg}
...
% 	\end{equation*}
\item sort of a measure of similarity between vectors, if they are the same length
\item example draw $(1/2,\sqrt{3}/2)$ and $(0,1)$
\item example draw $(1,0)$ and $(0,1)$
\item Example in slightly higher dimension $\langle (1,2,3), (5,0,2)\rangle= 5 + 0 + 6 = 11 $
...
\begin{frame}[t]{Quick computations}
\begin{columns}
\column{.45\textwidth}
\centering
What are the following? 
\begin{itemize}
	\item $\langle (1,1), (0,3) \rangle$ 
	\InstructorNotes{$=0+3=3$}
	\item $\langle (1,1), (3,2) \rangle$
	\InstructorNotes{$=3+2=5$}
	\item $\langle (2,3), (0,3) \rangle$
	\InstructorNotes{$=0+9=9$}
	\item $\langle (2,3), (3,2) \rangle$
	\InstructorNotes{$=6+6=12$}
\end{itemize}
\column{.45\textwidth}
\centering
\end{columns}
...
\end{equation*}
\begin{equation*}
	-2\sqrt 2 + \frac{\sqrt2}{18}\langle (X_1,X_2), (0,3) \rangle + \frac{\sqrt2}{6}\langle (X_1,X_2), (3,2) \rangle  = 0
\end{equation*}

\begin{columns}
\column{.25\textwidth}
\centering
\includegraphics[align = c, width = \textwidth]{SVM-Kernel-DesmosExample}
...
\InstructorList{
	\item work throught the formulation $\beta_0+\alpha_1\langle(X_1,X_2),(0,3)\rangle+\alpha_2\langle(X_1,X_2),(3,0)\rangle+\alpha_3\langle(X_1,X_2),(3,2)\rangle=0$ for the hyperplane
%   \item $f(1,1) = -2\sqrt2 + \frac{\sqrt2}{18}\langle (1,1), (0,3) \rangle + \frac{\sqrt2}{6}\langle (1,1), (3,2) \rangle $
%   \item $= -2\sqrt2 + \frac{\sqrt2}{18}\cdot 3 + \frac{\sqrt2}{6}\cdot 5 $
%   \item $= (-2 + \frac{3}{18} + \frac{5}{6})\sqrt 2 = -\sqrt 2$
	\item to get unique coefficients, I let $\alpha_2 =0$. other options are too complicated
	\item test $f((3,1))=f((2,2))=0$ (points on hyper plane)
	\item test $f((1,1))=-\sqrt{2}$
	\item The point: representing linear classifier $f(x)$ just needs inner products
}
...
% {
% \setbeamercolor{background canvas}{bg=Apricot!25}
% \begin{frame}[t]{What are the $\alpha_i$s?}
% \begin{columns}
% \column{.35\textwidth}
% \centering
% \begin{tabular}{c|c}
% 	Data point   & $\alpha_i$ \\ \hline 
% 	$(3,4)$      &   \phantom{XXXXXXXXX}         \\
% 	&\\
% 	$(2.5,3.5)$  &            \\
% 	&\\
% 	$(3,2)$      &            \\
...
% \begin{equation*}
% 	-2\sqrt 2 + \frac{\sqrt2}{18}\langle (X_1,X_2), (0,3) \rangle + \frac{\sqrt2}{6}\langle (X_1,X_2), (3,2) \rangle 
% \end{equation*}
...
% \InstructorNotes{
% $\alpha_0=\dfrac{\sqrt2}{18}$, $\alpha_2=0$,$\alpha_3=\dfrac{\sqrt2}{6}$
% }
% \end{columns}
% \end{frame}
% }
...
\end{frame}
%--- Next Frame ---%
\begin{frame}[t]{Support Vector Machine}
\begin{columns}
\column{.45\textwidth}
\centering
\begin{equation*}
...
\centering
\InstructorList{
  \item Learning function above
  \item Choose $K$ in advance 
}
...
\begin{frame}[t]{Next time}
	...
	\includegraphics[align = c, width =.4 \textwidth]{Schedule-F25-3.png}

\vspace{.2in}
% Q of the day:

% Why would you want to use SVM rather than SVC?
\end{frame}
%--- Next Frame ---%
