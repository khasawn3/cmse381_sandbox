...
\begin{columns}
	\InstructorNotes{Frak, this needs a bit of smoothing out. Maybe only do one coding chunk, no reason to split in two. }
\column{.45\textwidth}
\centering
...
\textbf{Announcements:}
\begin{itemize}
	\item Homework 7 Due Sunday 
\end{itemize}

\column{.45\textwidth}
\centering

\includegraphics[align = c, width = .8\textwidth]{Day01-Schedule-Test3}
...
\begin{itemize}
	...
	\item Return the average (or majority vote) of those as test prediction
\end{itemize}
\column{.45\textwidth}
\centering
...
{
\setbeamercolor{background canvas}{bg=Apricot!25}
\begin{frame}[t]{Coding example for random forests}
\begin{columns}
\column{.45\textwidth}
\centering
\column{.45\textwidth}
